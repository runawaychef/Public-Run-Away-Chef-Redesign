...
async function refreshPlanModalPrices() {
    ...
    if (!service) { showInfo('DEBUG v2: getBillingService() = null'); return; } // API недоступен здесь — оставляем статичные цены, кнопки остаются disabled ("Скоро будет доступно")

    let details;
    try {
        details = await service.getDetails([PLAN_PRODUCTS.light.sku, PLAN_PRODUCTS.full.sku]);
    } catch (e) {
        showInfo('DEBUG v2 getDetails error: ' + (e && e.message ? e.message : String(e)));
        return;
    }
    if (!details || !details.length) { showInfo('DEBUG v2: getDetails returned empty, length=' + (details ? details.length : 'null')); return; }
