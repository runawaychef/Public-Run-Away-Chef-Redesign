...
// Сигнализирует, что действительно есть что показать пользователю: либо
// экран входа (showAuthScreen), либо готовый авторизованный интерфейс —
// в том числе окно приветствия для нового пользователя (см. вызовы
// revealAppReady() в auth.js). Отдельно от initAuth()/_doShowAuthedApp()
// специально: при входе через Google initAuth() может резолвиться раньше,
// чем реально придёт событие onAuthStateChange('SIGNED_IN') и построится
// интерфейс — раньше на этот момент ориентировался сплэш-экран, из-за чего
// иногда успевал скрыться и показать пользователю недорисованное приложение
// ДО того, как появлялось окно приветствия (пойман на живом тестировании
// 16.07.2026). Теперь сплэш ждёт именно этот промис.
let _resolveAppReady = null;
...
setTimeout(revealAppReady, 8000);
